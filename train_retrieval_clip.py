...
class CLIPDataset(torch.utils.data.Dataset):
    ...
    def hide_region(self, image, bboxes):
        ...
        if self.args.hide_true_bbox == 2 or self.args.hide_true_bbox == 5 or self.args.hide_true_bbox == 7: #highlight mode
        ...
        for bbox in bboxes:
            ...
            elif self.args.hide_true_bbox == 2 or self.args.hide_true_bbox == 5 or self.args.hide_true_bbox == 7: # highlight mode
        ...
        if self.args.hide_true_bbox in [2, 3, 5, 6, 7]:
    ...
    def image_to_torch_tensor(self, image, save=False):
    ...
    def __getitem__(self, idx, save=None, save_extra=False):
        ...
        image = self.image_to_torch_tensor(image, save = save if save_extra else None)

        if save and save_extra:
            print(image.shape)
            save_image(image, save + '_postprocess.png')
...
def parse_args():
    ...
    parser.add_argument('--hide_true_bbox',
                        ...
                        help='0=plain, 1=hide true, 2=highlight true, 3=blackout all but true, 4=clue as image, 5=highlight + clue-inf together in text, 6=positiononly, 7=highlighting and only clue in text')
    ...
    parser.add_argument('--widescreen_processing',
                        type=int,
                        help='if 1, then we will run CLIP twice over each image twice to get a bigger field of view, if 2 we will squarepad (less computation)',
                        default=1,
                        choices=[0,1,2])
...
def clip_forward(model, image, text, only_features=True, image_dropout=None, text_dropout=None):
    ...
    if image_dropout:
        image_features = image_dropout(image_features)
...
def clip_forward_text_only(model, clue, inference, only_features=True):
    ...
    # cosine similarity as logits
    logit_scale = model.logit_scale.exp()
    logits_per_clue = logit_scale * clue_features @ inference_features.t()
    logits_per_inference = logits_per_clue.t()

    # shape = [global_batch_size, global_batch_size]
    return logits_per_clue, logits_per_inference, clue_features, inference_features
...
def main():
    ...
    with open(args.train) as f:
        train = json.load(f)
        #tmp = CLIPDataset(train, args, training=False)
        #tmp.__getitem__(88, save='position_only', save_extra=True)
        #quit()
    ...
    for i in range(3):
        val.dataset.__getitem__(i, save='method={}_idx={}'.format(args.hide_true_bbox, i))

    if args.test is None:
        args.test = args.val
        print('Test not specified. Outputting full results on validation set instead')

    with open(args.test) as f:
        test = json.load(f)
        test = torch.utils.data.DataLoader(
            CLIPDataset(test, args, training=False),
            batch_size=args.batch_size, num_workers=args.workers_dataloader, shuffle=False, worker_init_fn=worker_init_fn)
